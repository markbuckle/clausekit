...
function toModelCallError(err: unknown, userMessage: string): ModelCallError {
  const status = err instanceof Anthropic.APIError ? (err.status ?? 502) : 500;
  return new ModelCallError(userMessage, status);
}
...
function validateEdit(raw: unknown, documentText: string): SuggestedEdit | null {
  ...
  if (!documentText.includes(parsed.data.originalText)) {
    console.log(
      `[ask] dropped edit: originalText is not a verbatim substring (clauseRef=${parsed.data.clauseRef})`
    );
    return null;
  }
  return parsed.data;
...
function validateLadder(raw: unknown): LadderRung[] {
  ...
  for (const entry of raw) {
    const parsed = ladderRungSchema.safeParse(entry);
    if (parsed.success) rungs.push(parsed.data);
  }
  return rungs;
}
...
function validateTerms(raw: unknown, documentText: string): AnalyzedTerm[] {
  ...
  for (const entry of raw) {
    ...
    if (typeof t.clauseRef !== "string" || typeof t.currentText !== "string") continue;

    if (t.currentText.length === 0 || !documentText.includes(t.currentText)) {
      console.log(
        `[negotiate] dropped term: currentText is not a verbatim substring (clauseRef=${t.clauseRef})`
      );
      continue;
    }
...
export async function runAsk(input: AskInput): Promise<AskOutput> {
  ...
  if (toolUse && toolUse.input && typeof toolUse.input === "object") {
    const input2 = toolUse.input as Record<string, unknown>;
    answer = typeof input2.answer === "string" ? input2.answer.trim() : "";
    const validRefs = documentRefs(documentText);
    if (Array.isArray(input2.citations)) {
      citations = [
        ...new Set(
          input2.citations.filter((c): c is string => typeof c === "string" && validRefs.has(c))
        ),
      ];
    }
    edit = validateEdit(input2.edit, documentText) ?? undefined;
  }
...
export async function runNegotiate(input: NegotiateInput): Promise<NegotiateOutput> {
  ...
  try {
    ...
    throw toModelCallError(err, "Failed to build the negotiation brief.");
  }
