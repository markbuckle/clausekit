// Types for the seeded lease fixture. Pure data, no Office.js/React/service imports, so it can be consumed by the in-browser mock, the playground, and (later) the backend.

// One clause of the lease, authored as a locatable scope rather than a label. `ref` is a stable locator (e.g. "§5") the mock resolves to this span so `applyTrackedChange` can search within the clause's `text`.
...
// One position on a negotiation ladder, ordered landlord-favorable to tenant-favorable; each carries the actual redline `language` plus a one-line `rationale`.
...
// Curation sidecar for one contested clause, kept separate from the prose and never fed to the LLM as an answer key. The first rung of `fallbackLadder` echoes the clause's current (off-market) language; later rungs walk toward market and the counterparty.
