...
// Best-effort "§N → heading" map from the document text. The lease formats clause headings as "§5. Rent Escalation"; documents that don't follow this just yield no headings.
...
// Owns the Negotiation Simulator: the chosen side, the brief (terms + headings), and the call to /api/negotiate, grounded in the document via the seam. Each term's ladder rungs apply through the same applyTrackedChange path as Ask.
export function useNegotiate(): UseNegotiate {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      // Fully typed end to end: data.terms is inferred from the backend's AppRouter (zod schemas) and must remain assignable to the local AnalyzedTerm vocabulary, so drift fails at compile time.
